...
//		workername = user.getCheckname();
		...
		vacationorextraworklistdata = new String[] {"未打卡","换休","病假","年假","事假","产假","丧假"};				
		initialize();
	...
	private void initialize() {
		...
		detailstype.setModel(new DefaultComboBoxModel(new String[] {"全部", "换休", "年休", "事假","丧假", "产假", "陪护假", "未打卡"}));
		detailstype.setFont(new Font("Adobe 黑体 Std R", Font.PLAIN, 16));
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					if (((String)vacationorextrawork.getSelectedItem()).equals("加班")) {
						...
						detailstype.setModel(new DefaultComboBoxModel(new String[] {"全部","超时", "休息日", "公司统一","法定节假日"}));
